...
aic_scores <- model_orders %>% 
  dplyr::rowwise() %>% 
  mutate(family = ifelse(q == 0, "AR", ifelse(p == 0, "MA", "ARMA")),
         aic = try_default(AIC(Arima(co2_tsb$co2_ppm,
                                     order = c(p, 0, q))), default = NA, 
                           quiet = TRUE))

aic_bic_scores <- aic_scores %>% 
  mutate(bic = try_default(BIC(Arima(co2_tsb$co2_ppm,
                                     order = c(p, 0, q))), default = NA, 
                           quiet = TRUE))
...
kable(aic_bic_scores %>%
        ...
        group_by(family) %>% 
        top_n(-4, aic), 
      digits = 1, caption = "Top 4 models, per AIC score")
kable(aic_bic_scores %>% 
        arrange(bic) %>% 
        group_by(family) %>% 
        top_n(-4, bic), 
      digits = 1, caption = "Top 4 models, per BIC score")
```


```{r AIC Criteria}
model.aic<-co2_tsb %>%
  model(ARIMA(co2_ppm ~ 1 + pdq(0:12,0:3,0:12) + PDQ(0,0,0), ic="aic", stepwise=F,
              greedy=F))
...
```{r AICC Criteria}
model.aicc<-co2_tsb %>%
  model(ARIMA(co2_ppm ~ 1 + pdq(0:12,0:3,0:12) + PDQ(0,0,0), ic="aicc", stepwise=F,
              greedy=F))
...
```{r BIC Criteria}
model.bic<-co2_tsb %>%
  model(ARIMA(co2_ppm ~ 1 + pdq(0:12,0:3,0:12) + PDQ(0,0,0), ic="bic", stepwise=F,
              greedy=F))
