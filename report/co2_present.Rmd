...
and out of sample testing (fitting only on the training set) to find out the best. 

We seek the best ARIMA function across p = 1...12, d = 1 and q = 1...12. The reason to 
limit d is "Information criteria is not a good guide to select the order of differencing (d)".
...
The above results showcase the top models for each category. Let's first evaluate 
the performance of the top 5 **in-sample models** for other error metrics. 
...
print(paste("Error Metrics for p=",best.sas.insample$p_seq[1], " d=", best.sas.insample$d_seq[1], " d=", best.sas.insample$q_seq[1]))
print(paste("Resid Mean is ", mean(tail(aug$.resid,567)) ))
print(paste("MSE is ", mse_sa_insample))
...
aug<- fit %>% augment()
mse_sa_insample <- (accuracy(aug$.fitted, SA_co2_present$seasonally_adjusted_co2)[2])^2
print(paste("Error Metrics for p=",best.sas.insample$p_seq[2], " d=", best.sas.insample$d_seq[2], " d=", best.sas.insample$q_seq[2]))
print(paste("Resid Mean is ", mean(tail(aug$.resid,567)) ))
print(paste("MSE is ", mse_sa_insample))
...
aug<- fit %>% augment()
mse_sa_insample <- (accuracy(aug$.fitted, SA_co2_present$seasonally_adjusted_co2)[2])^2
print(paste("Error Metrics for p=",best.sas.insample$p_seq[3], " d=", best.sas.insample$d_seq[3], " d=", best.sas.insample$q_seq[3]))
print(paste("Resid Mean is ", mean(tail(aug$.resid,567)) ))
print(paste("MSE is ", mse_sa_insample))
...
mse_sa_insample <- (accuracy(aug$.fitted, SA_co2_present$seasonally_adjusted_co2)[2])^2
print(paste("Error Metrics for p=",best.sas.insample$p_seq[4], " d=", best.sas.insample$d_seq[4], " d=", best.sas.insample$q_seq[4]))
print(paste("Resid Mean is ", mean(tail(aug$.resid,567)) ))
print(paste("MSE is ", mse_sa_insample))
...
mse_sa_insample <- (accuracy(aug$.fitted, SA_co2_present$seasonally_adjusted_co2)[2])^2
print(paste("Error Metrics for p=",best.sas.insample$p_seq[5], " d=", best.sas.insample$d_seq[5], " d=", best.sas.insample$q_seq[5]))
print(paste("Resid Mean is ", mean(tail(aug$.resid,567)) ))
print(paste("MSE is ", mse_sa_insample))
```

Using the top 5 models from our AIC criterion, for seasonally adjusted in sample data, we observe all the MSE metrics are very close to `r mse_sa_insample`. We also observe that the residuals appear akin to a white noise pattern in their ts plot, as well as roughly normally distributed in their histogram. However, we observe a statistically significant value at lag 12 and 24 in the ACF plot. 

```{r, echo=FALSE, warning=FALSE, fig.dim=c(4,3)}
### NSA model with best AIC ARIMA
fit <- NSA_co2_present %>% model(ARIMA(co2_ppm ~ 0 + pdq(best.nsas.insample$p_seq[1],
...
qqnorm(aug$.resid, pch = 1, frame = FALSE, xlab = "Fitted", ylab = "Residual") 
fit %>% gg_tsresiduals()
mse_nsa_insample <- (accuracy(aug$.fitted, NSA_co2_present$co2_ppm)[2])^2
print(paste("Error Metrics for p=",best.nsas.insample$p_seq[1], " d=", best.nsas.insample$d_seq[1], " d=", best.nsas.insample$q_seq[1]))
print(paste("Resid Mean is ", mean(aug$.resid)))
print(paste("MSE is ", mse_nsa_insample))

fit <- NSA_co2_present %>% model(ARIMA(co2_ppm ~ 0 + pdq(best.nsas.insample$p_seq[2],
                                              best.nsas.insample$d_seq[2],
                                              best.nsas.insample$q_seq[2]) + PDQ(1,1,1), 
                             stepwise=FALSE, approximation=FALSE, method='ML')) 

aug<- fit %>% augment()
mse_nsa_insample <- (accuracy(aug$.fitted, NSA_co2_present$co2_ppm)[2])^2
print(paste("Error Metrics for p=",best.nsas.insample$p_seq[2], " d=", best.nsas.insample$d_seq[2], " d=", best.nsas.insample$q_seq[2]))
print(paste("Resid Mean is ", mean(aug$.resid)))
print(paste("MSE is ", mse_nsa_insample))

fit <- NSA_co2_present %>% model(ARIMA(co2_ppm ~ 0 + pdq(best.nsas.insample$p_seq[3],
                                              best.nsas.insample$d_seq[3],
                                              best.nsas.insample$q_seq[3]) + PDQ(1,1,1), 
                             stepwise=FALSE, approximation=FALSE, method='ML')) 

aug<- fit %>% augment()
mse_nsa_insample <- (accuracy(aug$.fitted, NSA_co2_present$co2_ppm)[2])^2
print(paste("Error Metrics for p=",best.nsas.insample$p_seq[3], " d=", best.nsas.insample$d_seq[3], " d=", best.nsas.insample$q_seq[3]))
print(paste("Resid Mean is ", mean(aug$.resid)))
print(paste("MSE is ", mse_nsa_insample))

fit <- NSA_co2_present %>% model(ARIMA(co2_ppm ~ 0 + pdq(best.nsas.insample$p_seq[4],
                                              best.nsas.insample$d_seq[4],
                                              best.nsas.insample$q_seq[4]) + PDQ(1,1,1), 
                             stepwise=FALSE, approximation=FALSE, method='ML')) 

aug<- fit %>% augment()
mse_nsa_insample <- (accuracy(aug$.fitted, NSA_co2_present$co2_ppm)[2])^2
print(paste("Error Metrics for p=",best.nsas.insample$p_seq[4], " d=", best.nsas.insample$d_seq[4], " d=", best.nsas.insample$q_seq[4]))
print(paste("Resid Mean is ", mean(aug$.resid)))
print(paste("MSE is ", mse_nsa_insample))

fit <- NSA_co2_present %>% model(ARIMA(co2_ppm ~ 0 + pdq(best.nsas.insample$p_seq[5],
                                              best.nsas.insample$d_seq[5],
                                              best.nsas.insample$q_seq[5]) + PDQ(1,1,1), 
                             stepwise=FALSE, approximation=FALSE, method='ML')) 

aug<- fit %>% augment()
mse_nsa_insample <- (accuracy(aug$.fitted, NSA_co2_present$co2_ppm)[2])^2
print(paste("Error Metrics for p=",best.nsas.insample$p_seq[5], " d=", best.nsas.insample$d_seq[5], " d=", best.nsas.insample$q_seq[5]))
print(paste("Resid Mean is ", mean(aug$.resid)))
print(paste("MSE is ", mse_nsa_insample))
```

Using the top 5 models from our AIC criterion, for seasonally adjusted in sample data, we observe a fit of `r mse_nsa_insample` for all the models. We also observe that the residuals appear akin to a white noise pattern in their ts plot, as well as roughly normally distributed in their histogram. The ACF plot shows statistically insignificant values for up through 24 lags. 

Next we will be evaluating the top 6 **out of sample performance** models for other error metrics. 
...
Evaluating the out of sample performance of the top 6 ARIMA models fit to the non-seasonally adjusted data, we see that the ARIMA model (12, 0, 6) produces the best out of sample performance, with the lowest mean squared error. This model produced the fourth highest AIC, but the difference in AIC score from the top ranked model and the fourth is relatively small. 
...
```{r log polynomial, echo=FALSE, warning=FALSE, fig.dim=c(4,3)}
...
We see statistically significant coefficients for the quadratic term at a very low p-value and an r-squared of 0.279, which means that only about a quarter of the variance in the data is explained by the model, worse than the linear model. The MSE is `r mse_quad` which is lower than the lowest MSE ARIMA out of sample performance of 1.88, indicating that this model may be more accurate at predicting future values than the ARIMA model. 
...
rownames(myData) <- c("Linear 420 FIRST CROSS", "SARIMA 420 FIRST CROSS", "NSA ARIMA 420 FIRST CROSS", 
                      "Linear 500 FIRST CROSS", "SARIMA 500 FIRST CROSS", "NSA ARIMA 500 FIRST CROSS",
                      "Linear 420 LAST CROSS", "SARIMA 420 LAST CROSS", "NSA ARIMA 420 LAST CROSS", 
                      "Linear 500 LAST CROSS", "SARIMA 500 LAST CROSS", "NSA ARIMA 500 LAST CROSS", 
                      "Jan 2122")
myData[1] <- "Apr 2022"
myData[14] <- "Mar 2023"
myData[27] <- "Mar 2022"
...
The columns in the above table are classified as point estimate, 80% low and high confidence, and
95% low and high confidence. The first 6 rows are segmented as when the 420/500 ppm
is crossed first and the last 6 rows reflects when it crosses the respective ppm levels last.
Three models are used 1) Linear 2) SARIMA 3) NSA ARIMA.
The values reflect the time the respective models crosses the threshold.

From what we have seen in the last few decades - the rise in $CO_2$ levels, the 
innovations that exaggerated those and the innovations that reduced it - and 
assuming similar trend continues, we are **95% confident** that the $CO_2$ levels would be in 
**546-733 ppm** range with a point estimate of **639 ppm**. 
